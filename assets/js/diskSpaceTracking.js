...
function getDiskStats(hostId) {
    ...
    var query = 
    "SELECT databasename, "
+"totalsizeMB as diskSize, "
+"allocatedspaceMB as allocatedspace, "
+"availablespaceMB as availablespace, "
+"usedspaceMB as usedspace, "
+"updated FROM (SELECT MAX(id) as id2, "
+"databasename as dev2,"
+"DATE(updated) as updated2 "
+"FROM tblSqlDBSize t "
+"WHERE hostid=" + hostId + " group by dev2,updated2) k1 LEFT JOIN tblSqlDBSize s on s.id=k1.id2;"
...
function getDiskStatsResults() {
    if (receiveReq.readyState == 4) {
		...
		for(var n=0;n < results.length;n++) {
			...
			devArray = getDevices(devArray,valObj['databasename']);
		}
		...
		for(var i=0 ; i < devArray.length;i ++) {
            ...
            var totalsize = getObjDiskElementFromArray(results, 'totalsizeMB',devArray[i], 0);
            var allocatedspace = getObjDiskElementFromArray(results, 'allocatedspaceMB',devArray[i], 0);
            var availablespace = getObjDiskElementFromArray(results, 'availablespaceMB',devArray[i], 0);
            var usedspace = getObjDiskElementFromArray(results, 'usedspaceMB',devArray[i], 0); 
			var jsonArrTimeStamp = getObjDiskElementFromArray(results,'updated',devArray[i],0);
            
            
			newGraph(document.getElementById('dsk'+i), "Disk Usage (" + devArray[i] + ")" ,"Usage (MB)",jsonArrTimeStamp,usedspace, devArray[i]+" - Used Space");
			//drawAvailableGraph(document.getElementById('dsk'+i), "Disk Usage (" + devArray[i] + ")" ,"Usage (MB)",jsonArrTimeStamp,availablespace, devArray[i]+" - Available Space");
			drawAllocationGraph(document.getElementById('dsk'+i), "Disk Usage (" + devArray[i] + ")" ,"Usage (MB)",jsonArrTimeStamp,allocatedspace, devArray[i]+" - Allocated Space");
			drawGraph(document.getElementById('dsk'+i), "Disk Usage (" + devArray[i] + ")" , "Usage (MB)",jsonArrTimeStamp,totalsize, devArray[i] + "- Total Size");
		}
    ...
    if (modalSiteId != "") {
        ...
        var query = "SELECT  "
            ...
            + "INNER JOIN InovoMonitor.tblSites ON InovoMonitor.tblSites.id = InovoMonitor.tblHosts.siteid  "
             ...
             + "WHERE InovoMonitor.tblHosts.enabled = 1 AND  InovoMonitor.tblAgent.agenttype in ('SQL Agent') AND InovoMonitor.tblSites.id = " + siteId + " "
        ...
        var query = "SELECT  "
            ...
            + "INNER JOIN InovoMonitor.tblAgent ON InovoMonitor.tblHosts.agentid = InovoMonitor.tblAgent.id "
            + "INNER JOIN InovoMonitor.tblSites ON InovoMonitor.tblSites.id = InovoMonitor.tblHosts.siteid  "
            //+ "WHERE InovoMonitor.tblHosts.enabled = 1 AND InovoMonitor.tblSites.id = " + siteId + " "
             + "WHERE InovoMonitor.tblHosts.enabled = 1 AND  InovoMonitor.tblAgent.agenttype in ('SQL Agent') AND InovoMonitor.tblSites.id = " + siteId + " "
            + "ORDER BY InovoMonitor.tblHosts.hostname asc;";
    }
...
function drawAllocationGraph(destControl, title, yLegend, xValues, yValues, traceName) {
    var data = [{
        y: yValues,
        x: xValues,
        name: traceName,
...
function drawAvailableGraph(destControl, title, yLegend, xValues, yValues, traceName) {
    var data = [{
        y: yValues,
        x: xValues,
        name: traceName,
